...
class ButterProductController extends ChangeNotifier {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        await userDocRef.update({
          ...
          'previousButterBalance':
              user.previousButterBalance, // Update previous balance
        });
  ...
  Future<void> clearAllButterDailyTasks(String email) async {
    try {
      ...
      if (userSnapshot.docs.isNotEmpty) {
        ...
        // Clear the butterDailyTasks list
        await userDoc.reference.update({
          'butterDailyTasks': [],
        });
      ...
      log('Error clearing butter daily tasks: $e');
    }
  }
  ...
  Future<void> saveAllUsersButterData() async {
    try {
      ...
      for (var userDoc in usersSnapshot.docs) {
        ...
        final butterDailyAmount =
            userDoc.data().containsKey('butterDailyAmount')
                ? userDoc['butterDailyAmount']
                : 0;
        final butterDailyQuantity =
            userDoc.data().containsKey('butterDailyQuantity')
                ? userDoc['butterDailyQuantity']
                : 0;
        final butterDailyTasks = userDoc.data().containsKey('butterDailyTasks')
            ? userDoc['butterDailyTasks']
            : [];
        final previousButterBalance =
            userDoc.data().containsKey('previousButterBalance')
                ? userDoc['previousButterBalance']
                : 0;

        if (butterDailyTasks != null && butterDailyTasks.isNotEmpty) {
          final allTimeDataRef =
              firebaseFirestore.collection('all_time_butter_data').doc();
          ...
          final butterData = {
            ...
            'previousButterBalance':
                previousButterBalance, // Save previous balance
            'timestamp': FieldValue.serverTimestamp(),
          };

          await allTimeDataRef.set(butterData);

          log('Butter data for user $email saved to all_time_butter_data successfully.');
        } else {
          log('No butter data found for user $email.');
        }
      }
    } catch (e) {
      log('Error saving all users\' butter data: $e');
    }
  }
  ...
  Future<void> deleteAllTimeButterData(String documentId) async {
    try {
      var documentRef =
          firebaseFirestore.collection('all_time_butter_data').doc(documentId);

      await documentRef.delete();

      log('All-time Butter data with ID $documentId deleted successfully.');
    } catch (e) {
      log('Error deleting all-time butter data: $e');
    }
  }
      ...
      if (confirm) {
        ...
        for (var doc in snapshot.docs) {
          await doc.reference.update({
            ...
            'butterDailyAmount': 0,
            'butterDailyQuantity': 0,
            'previousButterBalance': 0, // Reset previous balance
          });
        }
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("All Butter daily tasks have been cleared.")));
      }
    } catch (e) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text("Error: $e")));
